...
int main(int argc, char* argv[]) {
    ...
    std::vector<double> temp_sendbuf; 
    if (rank == 0) {
        temp_sendbuf.resize(n * n);
        for (std::size_t i = 0; i < n; ++i) {
            for (std::size_t j = 0; j < n; ++j) {
                temp_sendbuf[i * n + j] = A[i][j]; 
            }
        }
    }

    MPI_Scatterv(temp_sendbuf.data(), sendcounts.data(), displs.data(), MPI_DOUBLE,
